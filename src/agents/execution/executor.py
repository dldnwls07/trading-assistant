...
class OrderExecutor:
    ...
    async def execute_trade(self, ticker: str, side: str, quantity: int, price: float, agent_id: int = None) -> Dict[str, Any]:
        """
        단일 종목 매매 집행 (가상/실전 통합)
        """
        if self.is_virtual:
            return await self._execute_virtual_trade(ticker, side, quantity, price, agent_id)
        else:
    ...
    async def _execute_virtual_trade(self, ticker: str, side: str, quantity: int, price: float, agent_id: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if side == 'BUY':
                balance = await self.storage.get_virtual_balance(agent_id=agent_id)
                if balance < total_impact_krw:
                    return {"status": "error", "message": f"잔액 부족 (가상) - 필요: {total_impact_krw:,.0f}원"}
                
                await self.storage.update_virtual_balance(-total_impact_krw, agent_id=agent_id)
                await self.storage.update_virtual_position(ticker, quantity, executed_price, 'BUY', agent_id=agent_id)
                
            elif side == 'SELL':
                positions = await self.storage.get_virtual_positions(agent_id=agent_id)
                pos = next((p for p in positions if p['ticker'] == ticker), None)
                if not pos or pos['quantity'] < quantity:
                    return {"status": "error", "message": "보유 수량 부족 (가상)"}
                
                await self.storage.update_virtual_balance(-total_impact_krw, agent_id=agent_id) # Selling is negative impact on negative amount = addition
                await self.storage.update_virtual_position(ticker, quantity, executed_price, 'SELL', agent_id=agent_id)
            
            curr_sym = "$" if is_usd else "₩"
            current_balance = await self.storage.get_virtual_balance(agent_id=agent_id)
    ...
    async def calculate_position_size(self, ticker: str, price: float, agent_id: int = None) -> int:
        """비중 계산 (비동기 환율 반영)"""
        if self.is_virtual:
            await self.storage.initialize()
            balance = await self.storage.get_virtual_balance(agent_id=agent_id)
